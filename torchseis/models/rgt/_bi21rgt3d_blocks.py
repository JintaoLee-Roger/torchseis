...
from ...ops.chunked.interpolate import trilinear_interpolate_align_corners, interpolate3d_chunked
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
                 dilation=(1, 1), residual=True, norm=None, se=True):
        super(BasicBlock, self).__init__()
        self.conv1 = nn.Conv3d(inplanes, planes, kernel_size=3, stride=stride, padding=dilation[0], bias=False, dilation=dilation[0])
        self.bn1 = norm(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv3d(planes, planes, kernel_size=3, stride=1, padding=dilation[1], bias=False, dilation=dilation[1])
...
class Attention_block(nn.Module):
    ...
    def replace_norm(self):
        if isinstance(self.W_g[1], AdaptiveInstanceNorm3d):
            ...
            self.W_g[1] = AdaptiveInstanceNorm3d.from_instance_norm(self.W_g[1])
            self.W_x[1] = AdaptiveInstanceNorm3d.from_instance_norm(self.W_x[1])
            self.psi[1] = AdaptiveInstanceNorm3d.from_instance_norm(self.psi[1])
...
class UP(nn.Sequential):
    def __init__(self, in_c_g, in_c_x, out_c):
        ...
        # conventional conv
        self.conv = nn.Conv3d(num_input_features, out_c, kernel_size=5, stride=1, padding=2, bias=False)
        ...
        for t in range(3):
            for i, j, k in itertools.product(range(nb_d), range(nb_h), range(nb_w)):
                ...
                with torch.no_grad():
                    ...
                    patchj = x1[:, :, p_d0*2:p_d1*2, p_h0*2:p_h1*2, p_w0*2:p_w1*2]
                    patchj = F.pad(patchj, padlist2, mode='constant', value=0)

                    # attention
                    patchj = self.atte.chunked_forward(patchi, patchj, padlist2, t, indices2)
                    if t < 2:
                        continue
...
class _FuseOps:
    def _layer012(self, x: Tensor, bsize: int = 64):
        if not isinstance(self.layer0[1], AdaptiveInstanceNorm3d):
            self.layer0[1] = AdaptiveInstanceNorm3d.from_instance_norm(self.layer0[1])
        if not isinstance(self.layer1[1], AdaptiveInstanceNorm3d):
            self.layer1[1] = AdaptiveInstanceNorm3d.from_instance_norm(self.layer1[1])
        if not isinstance(self.layer2[1], AdaptiveInstanceNorm3d):
            self.layer2[1] = AdaptiveInstanceNorm3d.from_instance_norm(self.layer2[1])
                ...
                with torch.no_grad():
                    ...
                    if t==0:
                        ...
                        if self.fused:
                            ...
                            self.layer0[0] = ConvInstanceNorm3d(self.layer0[0], self.layer0[1], mean, var)
                    ...
                    if t==2:
                        ...
                        if self.fused:
                            ...
                            self.input_layer3[0] = ConvInstanceNorm3d(self.input_layer3[0], self.input_layer3[1], mean, var)
                            self.input_layer3[1].fused = True
                    patcho = self.layer2[1:](patcho)

                    out[:, :, o_d0:o_d1, o_h0:o_h1, o_w0:o_w1] = patcho[:, :, r_d0:r_d1, r_h0:r_h1, r_w0:r_w1]

        return out
    
    def _up5(self, up4, x, bsize=64):
        b, c, d, h, w = x.shape
        nb_d = (d + bsize - 1) // bsize
        nb_h = (h + bsize - 1) // bsize
        nb_w = (w + bsize - 1) // bsize
