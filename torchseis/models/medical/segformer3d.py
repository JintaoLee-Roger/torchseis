...
from torchseis.ops.chunked.conv import Conv3dChunked
from torchseis.ops.chunked.chunk_base import get_index
import itertools
...
class SegFormer3D(nn.Module):
    ...
    def forward(self, x, rank=0):
        ...
        if rank != 0 and self.training:
            raise RuntimeError("Only support eval mode")
        global OPTICAL_LEVEL
        old_level = OPTICAL_LEVEL
        OPTICAL_LEVEL = rank
        # embedding the input
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if OPTICAL_LEVEL > 0:
            # q, k, v: (B, H, N, Dh)
            # 注意：SDPA 默认按 1/sqrt(Dh) 缩放；若你确实要自定义缩放，可传 scale=...
            attn_out = F.scaled_dot_product_attention(
                q, k, v,
                attn_mask=None,
                dropout_p=self.attn_dropout.p if self.training else 0.0,
                is_causal=False,   # 若是解码器可改为 True
                scale=1.0 / math.sqrt(self.num_heads)  # 不建议这样；通常应为 1/sqrt(Dh)
            )

            # 还原回 (B, N, C)
            out = attn_out.transpose(1, 2).contiguous().reshape(B, N, C)
        else:
...
class DWConv(nn.Module):
    ...
    def forward(self, x):
        ...
        global OPTICAL_LEVEL
        if OPTICAL_LEVEL > 1 and not self.training:
            x = Conv3dChunked(self.dwconv, 64)(x)
        else:
            x = self.dwconv(x)
        # added batchnorm (remove it ?)
        x = self.bn(x)
...
class SegFormerDecoderHead(nn.Module):
    ...
    def fuse_decoder(self, c1, c2, c3, c4):
        b, c, d, h, w = c1.shape
        pad = 0
        ...
        for i, j, k in itertools.product(range(nb_d), range(nb_h), range(nb_w)):
            ...
            with torch.no_grad():
                patch1 = c1[:, :, p_d0:p_d1, p_h0:p_h1, p_w0:p_w1]
    ...
    def forward(self, c1, c2, c3, c4):
       ############## _MLP decoder on C1-C4 ###########
        ...
        global OPTICAL_LEVEL
        if OPTICAL_LEVEL == 2 and not self.training:
            _c = torch.cat([_c4, _c3, _c2, _c1], dim=1)
            x = Conv3dChunked(self.linear_fuse[0], 32)(_c)
            x = self.linear_fuse[1:](x)
            x = self.dropout(x)
            x = Conv3dChunked(self.linear_pred, 32)(x)

        elif OPTICAL_LEVEL == 3 and not self.training:
            x = self.fuse_decoder(_c4, _c3, _c2, _c1)
        else:
...
if __name__ == "__main__":
    ...
    OPTICAL_LEVEL:
        0 : raw
        1 : use F.scaled_dot_product_attention
        2 : use ChunkConv
        3 : fuse the decoder
    """
